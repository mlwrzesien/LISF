...
`Snowmodel topo-veg data source:` specifies the data format of SnowModel (by Glen Liston) parameter files. Current option is "Grads_binary".

`Snowmodel topo-veg map:` specifies specifies the location of SnowModel (by Glen Liston) parameter files. These files contain vegetation- and topo-based parameter information that can be used in a SnowModel (SM) model run.

`Snowmodel topo-veg map projection:` specfies the general SnowModel parameter map projection.  [red]#What projections are available?#

`Snowmodel topo-veg spatial transform:` specifies the grid transform option. Currently, none, neighbor or average are available options.

`SnowModel topo-veg parameter fill option:` specifies the general SnowModel parameter fill option.

.Example _ldt.config_ entry
....
Snowmodel topo-veg data source:  Grads_binary    
Snowmodel topo-veg map:          ./topo_vege.gdat
Snowmodel topo-veg spatial transform:  none       
Snowmodel topo-veg map projection:    latlon
