...
!BOP
! !ROUTINE: noahmp401_qc_sweobs
! \label{noahmp401_qc_sweobs}
!
! !REVISION HISTORY:
...
!
! !INTERFACE:
subroutine noahmp401_qc_sweobsPMW(n,k,OBS_State)
! !USES:
  use ESMF
  use LIS_coreMod
...
!  prior to data assimilation. Here the swe observations
!  are flagged when LSM indicates that (1) rain is falling (2)
!  ground is fully or partially covered with swe. 
!  
!  The arguments are: 
!  \begin{description}
...
!EOP
  ...
  real                     :: sweeqv(LIS_rc%npatch(n,LIS_rc%lsm_index))
  real                     :: sweeqv_obs(LIS_rc%obs_ngrid(k))

  call ESMF_StateGet(OBS_State,"Observation01",obs_swe_field,rc=status)
  call LIS_verify(status,&
       "ESMF_StateGet failed in noahmp401_qc_sweobsPMW")
  call ESMF_FieldGet(obs_swe_field,localDE=0,farrayPtr=sweobs,rc=status)
  call LIS_verify(status,&
       "ESMF_FieldGet failed in noahmp401_qc_sweobsPMW")

  snowliqtot = 0.0
  do t=1, LIS_rc%npatch(n,LIS_rc%lsm_index)
     !stc1(t) = noahmp401_struc(n)%noahmp401(t)%sstc(1) ! get swe/veg temp.
     stc1(t) = noahmp401_struc(n)%noahmp401(t)%tslb(1) ! get swe/veg temp.
     vegt(t) = LIS_surface(n,1)%tile(t)%vegt

     sweeqv(t) = noahmp401_struc(n)%noahmp401(t)%sneqv ! get swe/veg temp.
  ...
  call LIS_convertPatchSpaceToObsSpace(n,k,&
       LIS_rc%lsm_index,sweeqv,sweeqv_obs)

  do t=1,LIS_rc%obs_ngrid(k)
     if(sweobs(t).ne.LIS_rc%udef) then
        if(fveg_obs(t).gt.0.2.and.vegt_obs(t).le.4) then
           sweobs(t) = LIS_rc%udef
!        elseif(vegt_obs(t).le.4) then !forest types
!           sweobs(t) = LIS_rc%udef
        elseif(vegt_obs(t).eq.LIS_rc%glacierclass) then !TML: Eliminate Glaciers
           sweobs(t) = LIS_rc%udef
!assume that swe will not form at 5 deg. celcius or higher ground temp.
...
!           sweobs(t) = LIS_rc%udef
       elseif(snowliqtot(t).ge.1.0) then
           sweobs(t) = LIS_rc%udef
       elseif(sweeqv_obs(t).ge.150.0) then
           sweobs(t) = LIS_rc%udef
       endif
     endif
  enddo

end subroutine noahmp401_qc_sweobsPMW
