...
#include "LIS_misc.h"
!BOP
! !ROUTINE: timeinterp_geis
! \label{timeinterp_geis}
!
! !REVISION HISTORY:
!
! 15 Nov 2023: Sujay Kumar; Initial Version in LIS!
! !INTERFACE:
subroutine timeinterp_geis(n,findex)
! !USES:
  use ESMF
...
!=== Interpolate Data in time      
  wt1=(geis_struc(n)%geistime2-LIS_rc%time)/ & 
       (geis_struc(n)%geistime2-geis_struc(n)%geistime1)
  wt2=1.0-wt1
...
! Loop over number of forcing ensembles:
  mfactor = LIS_rc%nensem(n)/geis_struc(n)%nIter
  ...
  do k=1,LIS_rc%ntiles(n)/mfactor
     do m=1,mfactor
        ...
        if(geis_struc(n)%metdata1(kk,3,index1).ne.LIS_rc%udef.and.&
            geis_struc(n)%metdata2(kk,3,index1).ne.LIS_rc%udef) then 
          swd(t) = geis_struc(n)%metdata1(kk,3,index1)*zw1+&
              geis_struc(n)%metdata2(kk,3,index1)*zw2
          ! In cases of small cos(zenith) angles, use linear weighting
          !  to avoid overly large weights

          if((swd(t).gt.geis_struc(n)%metdata1(kk,3,index1).and. & 
              swd(t).gt.geis_struc(n)%metdata2(kk,3,index1)).and. & 
              (czb.lt.0.1.or.cze.lt.0.1))then
             swd(t) = geis_struc(n)%metdata1(kk,3,index1)*swt1+ & 
                      geis_struc(n)%metdata2(kk,3,index1)*swt2
          endif
        endif
        ...
        if (swd(t).gt.LIS_CONST_SOLAR) then
          ...
          write(unit=LIS_logunit,fmt=*)'[WARN] data1=',geis_struc(n)%metdata1(kk,3,index1)
          write(unit=LIS_logunit,fmt=*)'[WARN] data2=',geis_struc(n)%metdata2(kk,3,index1)
  ...
  do k=1,LIS_rc%ntiles(n)/mfactor
     do m=1,mfactor
        ...
        index1 = LIS_domain(n)%tile(t)%index
        kk = LIS_get_iteration_index(n, k, index1, mfactor)
        if((geis_struc(n)%metdata1(kk,1,index1).ne.LIS_rc%udef).and.&
            (geis_struc(n)%metdata2(kk,1,index1).ne.LIS_rc%udef)) then 
          tmp(t) = geis_struc(n)%metdata1(kk,1,index1)*wt1+ & 
                   geis_struc(n)%metdata2(kk,1,index1)*wt2
        endif
     enddo
  enddo
  ...
  do k=1,LIS_rc%ntiles(n)/mfactor
     do m=1,mfactor
        ...
        index1 = LIS_domain(n)%tile(t)%index
        kk = LIS_get_iteration_index(n, k, index1, mfactor)
        if((geis_struc(n)%metdata1(kk,2,index1).ne.LIS_rc%udef).and.&
            (geis_struc(n)%metdata2(kk,2,index1).ne.LIS_rc%udef)) then 
          q2(t) =geis_struc(n)%metdata1(kk,2,index1)*wt1+ & 
                 geis_struc(n)%metdata2(kk,2,index1)*wt2
        endif
     enddo
  enddo
  ...
  do k=1,LIS_rc%ntiles(n)/mfactor
     do m=1,mfactor
        ...
        index1 = LIS_domain(n)%tile(t)%index
        kk = LIS_get_iteration_index(n, k, index1, mfactor)
        if((geis_struc(n)%metdata1(kk,4,index1).ne.LIS_rc%udef).and.&
            (geis_struc(n)%metdata2(kk,4,index1).ne.LIS_rc%udef)) then 
           lwd(t) =geis_struc(n)%metdata1(kk,4,index1)*wt1+ & 
                   geis_struc(n)%metdata2(kk,4,index1)*wt2
        endif
     enddo
  enddo
  ...
  do k=1,LIS_rc%ntiles(n)/mfactor
     do m=1,mfactor
        ...
        index1 = LIS_domain(n)%tile(t)%index
        kk = LIS_get_iteration_index(n, k, index1, mfactor)
        if((geis_struc(n)%metdata1(kk,5,index1).ne.LIS_rc%udef).and.&
            (geis_struc(n)%metdata2(kk,5,index1).ne.LIS_rc%udef)) then 
          uwind(t) = geis_struc(n)%metdata1(kk,5,index1)*wt1+ & 
                     geis_struc(n)%metdata2(kk,5,index1)*wt2
        endif
     enddo
  enddo
  ...
  do k=1,LIS_rc%ntiles(n)/mfactor
     do m=1,mfactor
        ...
        index1 = LIS_domain(n)%tile(t)%index
        kk = LIS_get_iteration_index(n, k, index1, mfactor)
        if((geis_struc(n)%metdata1(kk,6,index1).ne.LIS_rc%udef).and.&
            (geis_struc(n)%metdata2(kk,6,index1).ne.LIS_rc%udef)) then 
          psurf(t) =geis_struc(n)%metdata1(kk,6,index1)*wt1+ & 
                    geis_struc(n)%metdata2(kk,6,index1)*wt2
        endif
     enddo
  enddo

end subroutine timeinterp_geis
