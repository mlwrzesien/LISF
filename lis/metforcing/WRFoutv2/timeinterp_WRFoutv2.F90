...
!BOP
!
! !ROUTINE: timeinterp_WRFoutv2
!  \label{timeinterp_WRFoutv2}
! 
! !REVISION HISTORY: 
...
! 
! !INTERFACE:
subroutine timeinterp_WRFoutv2(n, findex)
...
!EOP
  ...
  btime=WRFoutv2_struc(n)%WRFouttime1
  call LIS_time2date(btime,bdoy,gmt1,byr,bmo,bda,bhr,bmn)
  btime=WRFoutv2_struc(n)%WRFouttime2
  call LIS_time2date(btime,bdoy,gmt2,byr,bmo,bda,bhr,bmn)
  
  !===  Interpolate Data in Time
  wt1=(WRFoutv2_struc(n)%WRFouttime2-LIS_rc%time)/&
       (WRFoutv2_struc(n)%WRFouttime2-WRFoutv2_struc(n)%WRFouttime1)
  wt2=1.0-wt1
  ...
  call LIS_verify(status)
   ...
   mfactor = LIS_rc%nensem(n)/WRFoutv2_struc(n)%nIter
   ...
   do k=1,LIS_rc%ntiles(n)/mfactor
     do m=1,mfactor
       ...
       swd(t) = WRFoutv2_struc(n)%metdata1(kk,3,index1)*zw1
     
       if (swd(t) < 0) then
          write(LIS_logunit,*) '[ERR] WRFout V2 warning! SW radiation is negative!!'
          write(LIS_logunit,*) 'sw=', swd(t), '... negative'
          write(LIS_logunit,*) 'WRFout =', WRFoutv2_struc(n)%metdata2(kk,3,index1)
          call LIS_endrun
       end if
       if (swd(t).gt.LIS_CONST_SOLAR) then
          swd(t)=WRFoutv2_struc(n)%metdata1(kk,3,index1)
       endif
     end do
  enddo
  ...
  ! Time Averaged Longwave, Block Interpolation
   do k=1,LIS_rc%ntiles(n)/mfactor
     do m=1,mfactor
       ...
       tmp(t) = wt1 * WRFoutv2_struc(n)%metdata1(kk,1,index1) & 
              + wt2 * WRFoutv2_struc(n)%metdata2(kk,1,index1)
       q2(t) = wt1 * WRFoutv2_struc(n)%metdata1(kk,2,index1) & 
             + wt2 * WRFoutv2_struc(n)%metdata2(kk,2,index1)
       lwd(t) = WRFoutv2_struc(n)%metdata1(kk,4,index1)
       uwind(t) = wt1 * WRFoutv2_struc(n)%metdata1(kk,5,index1) & 
                + wt2 * WRFoutv2_struc(n)%metdata2(kk,5,index1)
       vwind(t) = wt1 * WRFoutv2_struc(n)%metdata1(kk,6,index1) & 
...
end subroutine timeinterp_WRFoutv2
