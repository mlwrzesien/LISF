...
!BOP
! 
! !ROUTINE: write_simSnowGlobeTuoobs
! \label{write_simSnowGlobeTuoobs}
! 
! !REVISION HISTORY: 
! 25Jan2008: Sujay Kumar; Initial Specification
! 
! !INTERFACE: 
subroutine write_simSnowGlobeTuoobs(n,k, OBS_State)
! !USES: 
  use ESMF
  use LIS_coreMod
  use LIS_logMod
...
!EOP
  ...
  call LIS_verify(status,&
       "ESMF_AttributeGet: Data Update Status failed in write_simSnowGlobeTuoobs")
  ...
  if(data_update) then 
     ...
     call ESMF_StateGet(OBS_State, "Observation01",smField, &
          rc=status)
     call LIS_verify(status, &
          "ESMF_StateGet failed in write_simSnowGlobeTuoobs")
     
     call ESMF_FieldGet(smField, localDE=0, farrayPtr=sweobs, rc=status)
     call LIS_verify(status,&
          "ESMF_FieldGet failed in write_simSnowGlobeTuoobs")


     if(LIS_masterproc) then 
        ftn = LIS_getNextUnitNumber()
        call snowGlobeTuo_sweobsname(n,k,obsname)        
...
end subroutine write_simSnowGlobeTuoobs

!BOP
! !ROUTINE: snowGlobeTuo_sweobsname
! \label{snowGlobeTuo_sweobsname}
! 
! !INTERFACE: 
subroutine snowGlobeTuo_sweobsname(n,k,obsname)
! !USES: 
...
end subroutine snowGlobeTuo_sweobsname
