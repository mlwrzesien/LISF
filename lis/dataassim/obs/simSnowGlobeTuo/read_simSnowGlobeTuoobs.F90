...
#include "LIS_misc.h"
!BOP
! !ROUTINE: read_simSnowGlobeTuoobs
!  \label{read_simSnowGlobeTuoobs}
!
! !REVISION HISTORY:
!  21Jun2006: Sujay Kumar; Initial Specification
!
! !INTERFACE: 
subroutine read_simSnowGlobeTuoobs(n, k, OBS_State, OBS_Pert_State) 
  ! !USES:
  ...
  use simSnowGlobeTuoobs_module, only : SnowGlobeTuo_struc
...
!EOP
  ...
  real                :: sweobs(SnowGlobeTuo_struc(n)%nc,SnowGlobeTuo_struc(n)%nr)
  ...
  call simSnowGlobeTuoSWE_filename(name,sweobsdir,&
...
!     print *,'reading SimObs file' 
  ...
  if (readflag) then 
     ...
     allocate(swe_data_b(SnowGlobeTuo_struc(n)%nc*SnowGlobeTuo_struc(n)%nr))
     allocate(var(SnowGlobeTuo_struc(n)%nc,SnowGlobeTuo_struc(n)%nr))
     allocate(swe1d(SnowGlobeTuo_struc(n)%nc*SnowGlobeTuo_struc(n)%nr))
     ...
     call LIS_verify(nf90_get_var(ftn,sweid,sweobs, &
          count=(/SnowGlobeTuo_struc(n)%nc,SnowGlobeTuo_struc(n)%nr,1/),&
          start=(/1,1,1/)),&
...
!-------------------------------------------------------------------------- 
        ...
        do r=1, SnowGlobeTuo_struc(n)%nr
           do c=1, SnowGlobeTuo_struc(n)%nc
              if(sweobs(c,SnowGlobeTuo_struc(n)%nr-r+1).ge.0) then
                 swe1d(c+(r-1)*SnowGlobeTuo_struc(n)%nc) = &
                      sweobs(c,SnowGlobeTuo_struc(n)%nr-r+1)
                 swe_data_b(c+(r-1)*SnowGlobeTuo_struc(n)%nc)=.true.
              endif
           enddo
        enddo
        deallocate(var)
...
!end MLW edits
     ...
     call lsmdaqcobsstate(trim(LIS_rc%lsm)//"+" &
          //trim(LIS_SnowGlobeTuoSWEId)//char(0), n, k, OBS_state)
...
end subroutine read_simSnowGlobeTuoobs

subroutine simSnowGlobeTuoSWE_filename(name, ndir, yr, mo,da,hr,mn)
...
end subroutine simSnowGlobeTuoSWE_filename
