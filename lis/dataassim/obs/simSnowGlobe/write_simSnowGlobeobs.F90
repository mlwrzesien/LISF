...
!BOP
! 
! !ROUTINE: write_simSnowGlobeobs
! \label{write_simSnowGlobeobs}
! 
! !REVISION HISTORY: 
! 25Jan2008: Sujay Kumar; Initial Specification
! 
! !INTERFACE: 
subroutine write_simSnowGlobeobs(n,k, OBS_State)
! !USES: 
  use ESMF
  use LIS_coreMod
  use LIS_logMod
...
!EOP
  ...
  call LIS_verify(status,&
       "ESMF_AttributeGet: Data Update Status failed in write_simSnowGlobeobs")
  ...
  if(data_update) then 
     ...
     call ESMF_StateGet(OBS_State, "Observation01",smField, &
          rc=status)
     call LIS_verify(status, &
          "ESMF_StateGet failed in write_simSnowGlobeobs")
     
     call ESMF_FieldGet(smField, localDE=0, farrayPtr=sweobs, rc=status)
     call LIS_verify(status,&
          "ESMF_FieldGet failed in write_simSnowGlobeobs")


     if(LIS_masterproc) then 
        ftn = LIS_getNextUnitNumber()
        call snowGlobe_sweobsname(n,k,obsname)        
...
end subroutine write_simSnowGlobeobs

!BOP
! !ROUTINE: snowGlobe_sweobsname
! \label{snowGlobe_sweobsname}
! 
! !INTERFACE: 
subroutine snowGlobe_sweobsname(n,k,obsname)
! !USES: 
...
end subroutine snowGlobe_sweobsname
