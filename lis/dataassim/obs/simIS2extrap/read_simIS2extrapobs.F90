...
#include "LIS_misc.h"
!BOP
! !ROUTINE: read_simIS2extrapobs
!  \label{read_simIS2extrapobs}
!
! !REVISION HISTORY:
!  21Jun2006: Sujay Kumar; Initial Specification
!
! !INTERFACE: 
subroutine read_simIS2extrapobs(n, k, OBS_State, OBS_Pert_State) 
  ! !USES:
  ...
  use simIS2extrapobs_module, only : IS2extrap_struc
...
!EOP
  ...
  real                :: snwdobs(IS2extrap_struc(n)%nc,IS2extrap_struc(n)%nr)
  ...
  call simIS2extrapSNWD_filename(name,snwdobsdir,&
...
!     print *,'reading SimObs file' 
  ...
  if (readflag) then 
     ...
     allocate(snwd_data_b(IS2extrap_struc(n)%nc*IS2extrap_struc(n)%nr))
     allocate(var(IS2extrap_struc(n)%nc,IS2extrap_struc(n)%nr))
     allocate(snwd1d(IS2extrap_struc(n)%nc*IS2extrap_struc(n)%nr))
     ...
     call LIS_verify(nf90_get_var(ftn,snwdid,snwdobs, &
          count=(/IS2extrap_struc(n)%nc,IS2extrap_struc(n)%nr,1/),&
          start=(/1,1,2/)),&
...
!-------------------------------------------------------------------------- 
        ...
        do r=1, IS2extrap_struc(n)%nr
           do c=1, IS2extrap_struc(n)%nc
              if(snwdobs(c,IS2extrap_struc(n)%nr-r+1).ge.0) then
                 snwd1d(c+(r-1)*IS2extrap_struc(n)%nc) = &
                      snwdobs(c,IS2extrap_struc(n)%nr-r+1)
                 snwd_data_b(c+(r-1)*IS2extrap_struc(n)%nc)=.true.
              endif
           enddo
        enddo
        deallocate(var)
...
!end MLW edits
     ...
     call lsmdaqcobsstate(trim(LIS_rc%lsm)//"+" &
          //trim(LIS_IS2extrapSNWDId)//char(0), n, k, OBS_state)
...
end subroutine read_simIS2extrapobs

subroutine simIS2extrapSNWD_filename(name, ndir, yr, mo,da,hr,mn)
...
end subroutine simIS2extrapSNWD_filename
