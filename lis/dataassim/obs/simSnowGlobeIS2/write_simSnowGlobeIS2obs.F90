...
!BOP
! 
! !ROUTINE: write_simSnowGlobeIS2obs
! \label{write_simSnowGlobeIS2obs}
! 
! !REVISION HISTORY: 
! 25Jan2008: Sujay Kumar; Initial Specification
! 
! !INTERFACE: 
subroutine write_simSnowGlobeIS2obs(n,k, OBS_State)
! !USES: 
  use ESMF
  use LIS_coreMod
  use LIS_logMod
...
!EOP
  ...
  call LIS_verify(status,&
       "ESMF_AttributeGet: Data Update Status failed in write_simSnowGlobeIS2obs")
  ...
  if(data_update) then 
     ...
     call ESMF_StateGet(OBS_State, "Observation01",smField, &
          rc=status)
     call LIS_verify(status, &
          "ESMF_StateGet failed in write_simSnowGlobeIS2obs")
     
     call ESMF_FieldGet(smField, localDE=0, farrayPtr=snwdobs, rc=status)
     call LIS_verify(status,&
          "ESMF_FieldGet failed in write_simSnowGlobeIS2obs")


     if(LIS_masterproc) then 
        ftn = LIS_getNextUnitNumber()
        call snowGlobeIS2_snwdobsname(n,k,obsname)        
...
end subroutine write_simSnowGlobeIS2obs

!BOP
! !ROUTINE: snowGlobeIS2_snwdobsname
! \label{snowGlobeIS2_snwdobsname}
! 
! !INTERFACE: 
subroutine snowGlobeIS2_snwdobsname(n,k,obsname)
! !USES: 
...
end subroutine snowGlobeIS2_snwdobsname
