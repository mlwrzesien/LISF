...
!BOP
! 
! !ROUTINE: write_simSnowGlobePMWobs
! \label{write_simSnowGlobePMWobs}
! 
! !REVISION HISTORY: 
! 25Jan2008: Sujay Kumar; Initial Specification
! 
! !INTERFACE: 
subroutine write_simSnowGlobePMWobs(n,k, OBS_State)
! !USES: 
  use ESMF
  use LIS_coreMod
  use LIS_logMod
...
!EOP
  ...
  call LIS_verify(status,&
       "ESMF_AttributeGet: Data Update Status failed in write_simSnowGlobePMWobs")
  ...
  if(data_update) then 
     ...
     call ESMF_StateGet(OBS_State, "Observation01",smField, &
          rc=status)
     call LIS_verify(status, &
          "ESMF_StateGet failed in write_simSnowGlobePMWobs")
     
     call ESMF_FieldGet(smField, localDE=0, farrayPtr=sweobs, rc=status)
     call LIS_verify(status,&
          "ESMF_FieldGet failed in write_simSnowGlobePMWobs")


     if(LIS_masterproc) then 
        ftn = LIS_getNextUnitNumber()
        call snowGlobePMW_sweobsname(n,k,obsname)        
...
end subroutine write_simSnowGlobePMWobs

!BOP
! !ROUTINE: snowGlobePMW_sweobsname
! \label{snowGlobePMW_sweobsname}
! 
! !INTERFACE: 
subroutine snowGlobePMW_sweobsname(n,k,obsname)
! !USES: 
...
end subroutine snowGlobePMW_sweobsname
